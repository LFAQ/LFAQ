...
bool tree::birth(size_t nid, size_t v, size_t c, double ml, double mr)
{
	...
	if (np == 0) 
	{
		...
		flog.mf_Input("Error:\terror in birth: bottom node not found\n");
		return false; //did not find note with that nid
	}
	if (np->l)
	{
		cout << "error in birth: found node has children\n";
		flog.mf_Input("Error:\terror in birth: found node has children\n");
...
bool tree::death(size_t nid, double mu)
{
	...
	if (nb == 0)
	{
		...
		flog.mf_Input("Error:\terror in death, nid invalid\n");
		return false;
	}
	if (nb->isnog()) 
	{
	...
	else 
	{
		...
		flog.mf_Input("Error:\terror in death, node is not a nog node\n");
		return false;
	}
}
//--------------------
...
void tree::pr(bool pc) const
{
	...
	if (pc && (ntype() == 't'))
	{
		...
		flog.mf_Input("tree size: " + fSize_t2String(treesize()) + "\n");
	...
	cout << sp << "depth: " << depth();
	cout << endl;
	flog.mf_Input(pad + "id: " + fSize_t2String(id));
	flog.mf_Input(sp + "(v,c): " + fSize_t2String(v) + sp + fSize_t2String(c));
	flog.mf_Input(sp + "mu: " + fDouble2String(mu));
	flog.mf_Input(sp + "type: " + ntype());
	flog.mf_Input(sp + "depth: " + fSize_t2String(depth()));
	flog.mf_Input("\n");
	if (pc) 
	{
		if (l) {
